...
export const getPartialMigration = (actions) => {
  ...
  const writeCommand = (action, pars = [], addTransaction = false) => {
    pars = [
      action.tableName,
      ...pars,
      addTransaction ? addTransactionToOptions(action.options) : '{ transaction }',
    ];
    let params = '';
    pars.forEach((par) => {
  ...
  actions.forEach((action) => {
    ...
    if (action) {
      commands.push(writeCommand(action, commandPars, addTransaction));
