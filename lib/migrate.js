...
const writeMigration = (
  ...
  let _commandsUp = `var migrationCommands = (transaction) => {return [ \n${migration.commandsUp.join(
    ', \n',
  )} \n];};\n`;
  let _commandsDown = `var rollbackCommands = (transaction) => {return [ \n${migration.commandsDown.join(
    ', \n',
  )} \n];};\n`;
  const _actions = ` * ${migration.consoleOut.join('\n * ')}`;

  _commandsUp = format(_commandsUp, { parser: 'babel' });
  _commandsDown = format(_commandsDown, { parser: 'babel' });
  const info = {
    revision,
    name,
    created: new Date(),
...
const executeMigration = async (queryInterface, filename, useTransaction, pos, rollback, cb) => {
  const mig = await import(filename);
