      ...
      case 'E':
        {
          ...
          if (df.path[1] === 'indexes') {
            Object.values(df.rhs).forEach((val) => {
              // const index = clone(df.rhs[key]);
              actions.push({
                actionType: 'addIndex',
                tableName,
                fields: df.rhs[val].fields,
                options: df.rhs[val].options,
                depends: [tableName],
              });
            });

            Object.values(df.lhs).forEach((val) => {
              // const index = clone(df.lhs[key]);
              actions.push({
                actionType: 'removeIndex',
                tableName,
                fields: df.lhs[val].fields,
                options: df.lhs[val].options,
                depends: [tableName],
              });
            });
          }
